...
#[tonic::async_trait]
pub trait RedisBox {
    async fn init_cache_conn(&mut self, db_cf: CfDatabase) -> Result<bool, RedisError>;
    async fn get_room_data(mut self, room_key: &str) -> Result<GameDataSet, RedisError>;
    async fn set_room_data(mut self, _room: GameDataSet) -> Result<(), RedisError>;
}
// impl Copy for RedisBox{}
#[tonic::async_trait]
impl RedisBox for GameDuelServiceBackend {
    async fn init_cache_conn(&mut self, db_cf: CfDatabase) -> Result<bool, RedisError> {
        println!("hi i am in redis-box");
        ...
        println!("{}", addr);
        let result = redis::Client::open(addr)?;
